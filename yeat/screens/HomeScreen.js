...
export default class HomeScreen extends React.Component {
    ...
    componentDidMount() {
        ...
        Location.watchPositionAsync({
            ...
            _this.updateDiningHallsController(() => {
     ...
     */
    updateDiningHallsController(callback) {
    ...
    /* 
     * handles the sorting rules for dining hall distances
     */
    ...
    /*
     * accesses database to get user preferences if they are logged in
     */
    checkUserController(callback) {
        var _this = this;
    ...
    /*
     * accesses database and retrieves food items from dining halls
     */
    getFoodController(callback) {
        var foodarr = [];
        var _this = this;
    ...
    /*
     * apply the filterController according to the user's preferences
     *
    ...
    filterController() {
        ...
        // traverse through preferences to initialize filterControllering
        for (let [key, val] of Object.entries(this.state.preferences)) {
                ...
                firebase.database().ref(_this.state.hour + '/Vegan/').on('value', function (data) {
                    ...
                    data.forEach(function (item) {
                        ...
                        for (var i = 0; i < foodarr.length; i++) {
                            ...
                            var getFoodController = foodarr[i].items.find(function (element) {
                                return element.name === item.key;
                            });
                            if (getFoodController !== undefined) {
                                foodvega[i].items.push(getFoodController);
                            }
                        }
                ...
                firebase.database().ref(_this.state.hour + '/Vegetarian/').on('value', function (data) {
                    ...
                    data.forEach(function (item) {
                        ...
                        for (var i = 0; i < foodarr.length; i++) {
                            ...
                            if (getFoodController !== undefined) {
                                foodvege[i].items.push(getFoodController); //remove this item from food array
                            }
                        }
                        if (counter === data.numChildren()) {
            ...
            } else if (val === false && key !== 'vegan' && key !== 'vegetarian') { // restrictions and preferences (to filterController)
                firebase.database().ref(_this.state.hour).on('value', function (data) {
        ...
        this.setState({
            food: foodarr // update the filterControllered food array to the state attribute
        }, function () {
        ...
        return (
            ...
            <View style={styles.container}>
                <ScrollView style={styles.container} >
                    ...
                    {/* handles displaying the yeatiest food item */}
                    ...
                    <Card containerStyle={{ alignSelf: 'center', width: 325, height: 150, backgroundColor: '#39cbd6', borderRadius: 15, top:-5, marginTop: 0 }}
                        ...
                        {/* handles displaying the yuckiest food item */}
